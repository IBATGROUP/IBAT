...
async function atLogout() {
  ...
  try {
    populateReferralLinkinHTML("Кошелек не подключен");
  } catch (e) {
    console.log(e),
      console.error(
  ...
  try {
    populateReferrerinHTML("Кошелек не подключен");
  } catch (e) {
    console.log(e),
      console.error(
  ...
  manageTooltipWidth(),
    toggleChainName(!1),
    toggleConnectStatus(),
    populateUserAddress("Не подключен", !1);
    ...
    e.forEach((e) => {
      isUserConnected()
        ? (e.classList.remove("bg-red-400"),
          ...
          (e.innerHTML = "связано"))
        : ((e.style.background = "#f87171"), (e.innerHTML = "Не подключен"));
    });
}
...
async function buyToken() {
    toastError("🎊ПРЕДПРОДАЖА ПРОДАНА🎉");
    return;
  ...
  if (!isUserConnected())
    return (
      console.log("Пользователь не вошел в систему - подключите кошелек"),
      (n = "Кошелек не подключен"),
      void showErrMsg(o, t, n)
    );
  if (e <= 0)
    return (
      (n = "Введите действительную сумму от 0,1 до 500 BNB."),
      console.error(`dev: BNBAmount = ${e}`),
      void showErrMsg(o, t, n)
    );
  if (isNaN(e))
    return (
      (n = "Введите действительную сумму"),
      console.error(`dev: BNBAmount = ${e}`),
  ...
  if (s < e)
    return (
      (n = `Ваш баланс BNB: ${s}\n Мин. требуется = 0.1BNB + gas`),
      void toastError(n)
    );
  if (!isSaleOpen)
    return (
      console.log("Presale is not open"),
      (n = "Предпродажа еще не началась!"),
      void showErrMsg(o, t, n)
    );
  if (
    ...
    return (
      (n = "Минимальная сумма BNB должна быть 0,1 BNB."),
      void showErrMsg(o, t, n)
    );
  if (+e > 500)
    return (
      (n = "Максимальная сумма BNB составляет 500 BNB."),
      void showErrMsg(o, t, n)
    );
  let r,
    a = {
    ...
    try {
      if (!(await isValidCouponCode(l)))
        return void toastError(
          "❌Введите действующий код купона\nили удалите его - очистите поле купона."
        );
    } catch (e) {
      console.error(e),
  ...
  try {
    ...
    showTransactionLoader(!0),
      ...
      "undefined" == typeof toastSuccess
        ? alert(
            "✅Транзакция прошла успешно✅\nПожалуйста, импортируйте токен и проверьте свой кошелек."
          )
        : toastSuccess(
            "✅Транзакция прошла успешно.\nПожалуйста, импортируйте токен и проверьте свой кошелек.",
            1e4
    ...
    l &&
      ...
      updateCouponUserDB(r.transactionHash, o, e, n, l)
        .then(() => {
          toastSuccess("✅Купон успешно добавлен.", 1e4);
        })
        .catch((e) => {
          toastError("❌Не удалось добавить купон.", 1e4),
            console.error(e),
    ...
    if (
      ...
      return (
        (n = "Транзакция отклонена пользователем"), void showErrMsg(o, t, n)
      );
    (n = e.data.message),
      console.error(n),
      console.error("dev:buyToken() - Transaction cancelled - 2"),
      (n = "Недостаточно BNB в кошельке"),
      showErrMsg(o, t, n);
  }
}
    ...
    e.forEach((e) => {
      showAddressFied
        ? (e.setAttribute("type", "text"),
          (showAddressBtn.innerHTML = "СПРЯТАТЬ"))
        : (e.setAttribute("type", "password"),
          (showAddressBtn.innerHTML = "ПОКАЗЫВАТЬ"));
    });
}
...
//# sourceMappingURL=index.f6ecb009.js.map
